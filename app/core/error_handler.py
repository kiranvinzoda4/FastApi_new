...
class ErrorHandler:
    ...
    def handle_exception(e: Exception, context: str = "") -> HTTPException:
        ...
        if isinstance(e, ValidationError):
            logger.warning(
                f"Validation error{f' in {context}' if context else ''}: {e}"
            )
        ...
        if isinstance(e, IntegrityError):
            logger.error(
                f"Database integrity error{f' in {context}' if context else ''}: {e}"
            )
        ...
        logger.error(
            f"Unexpected error{f' in {context}' if context else ''}: {e}", exc_info=True
        )
        return HTTPException(
...
async def global_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    http_exc = ErrorHandler.handle_exception(
