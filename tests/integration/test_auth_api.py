...
class TestAuthAPI:
    ...
    def test_update_admin_profile(self, client: TestClient, auth_headers):
        ...
        response = client.put(
            "/admin/admin-profile", json=update_data, headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_change_password_success(self, client: TestClient, auth_headers):
        ...
        response = client.put(
            "/admin/admin-change-password", json=password_data, headers=auth_headers
        )
        assert response.status_code == 200
    ...
    def test_forgot_password_existing_user(self, client: TestClient, admin_user):
        ...
        response = client.put(
            "/admin/send-forgot-password-email", json={"email": admin_user.email}
        )
    ...
    def test_forgot_password_nonexistent_user(self, client: TestClient):
        ...
        response = client.put(
            "/admin/send-forgot-password-email",
